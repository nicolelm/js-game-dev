...
function slideLeft(){
	for(let r=0; r<rows; r++){
		...
		for(let c=0; c<columns; c++){
            ...
            //line for animation
            // if currentTile != to original tile apply animation
...
//event listener to capture touch in the screen and assign the x and y coordinates
...
document.addEventListener("touchend",(e) =>{
	...
	//check if the element triggered has a class name
	if(!e.target.className.includes("tile")){
		return;
	}

	//calculate the start and end coordinates to know the direction
